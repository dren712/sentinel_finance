  ...
  const defaultTimelineItems: TimelineItem[] = [
    {
      id: 'canon-01',
      time: '14:32',
      action: 'PROVN Evidence Anchored',
      amount: 'Audit Seal',
      status: 'SETTLED',
      statusLabel: 'Evidence Anchored',
      headline: 'PROVN Cryptographic Evidence Anchored',
      subheadline: `Anchored on-chain in EvidenceAccount PDA · Devnet TX ${formatAddress(APP_CONFIG.devnetTransactions.recordEvidenceTx, 6)}`,
      txSignature: APP_CONFIG.devnetTransactions.recordEvidenceTx,
      isRealDevnetTx: true,
      evidenceHash: `0x${sha256Hex('canon-01:RECORD_EVIDENCE:PROVN:SETTLED').slice(0, 32)}`,
      intentHash: `0x${sha256Hex('intent:canon-01:RECORD_EVIDENCE').slice(0, 32)}`,
      preStateHash: `sha256:${sha256Hex('prestate:canon-01:evidence:anchor').slice(0, 24)}`,
      beforeVsProposed: [
      ...
      checks: [
        ...
        { name: 'PromiseAccount PDA verified on-chain', passed: true },
        { name: 'SHA-256 evidence hash anchored on Devnet', passed: true },
        { name: 'Two-tier verification receipt sealed', passed: true },
      ],
      adaptationNarrative: {
        adaptedAction: 'Cryptographic SHA-256 evidence receipt committed on Solana Devnet',
        settlementTx: `Devnet Evidence TX: ${APP_CONFIG.devnetTransactions.recordEvidenceTx}`,
      },
    },
    {
      id: 'canon-02',
